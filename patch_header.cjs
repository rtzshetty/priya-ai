...
const startTag = '<header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">';
const endTag = '</header>';
...
if (startIndex !== -1 && endIndex !== -1) {
  const newHeaderStr = `<header className="absolute top-0 left-0 w-full flex flex-col md:flex-row justify-between items-start md:items-center z-20 shrink-0 px-4 py-3 md:px-12 md:py-6 gap-3 md:gap-0 pointer-events-none">
  ...
  content = content.substring(0, startIndex) + newHeaderStr + content.substring(endIndex);
  console.log("Header replaced successfully!");
} else {
  console.error("Header tags not found!");
}

const originalMainStr = `<main className="absolute inset-0 flex flex-row items-center justify-between w-full h-full z-10 overflow-hidden pt-20 pb-24 px-4 md:px-12 pointer-events-none">`;
const newMainStr = `<main className="absolute inset-0 flex flex-row items-center justify-between w-full h-full z-10 overflow-hidden pt-28 md:pt-20 pb-24 px-4 md:px-12 pointer-events-none">`;
content = content.replace(originalMainStr, newMainStr);

const originalChatAreaStr = `<div className="absolute inset-x-0 top-24 bottom-32 overflow-y-auto px-6 md:px-12 pointer-events-none z-10 custom-scrollbar">`;
const newChatAreaStr = `<div className="absolute inset-x-0 top-28 md:top-24 bottom-32 md:bottom-32 overflow-y-auto px-4 sm:px-6 md:px-12 pointer-events-none z-10 custom-scrollbar">`;
content = content.replace(originalChatAreaStr, newChatAreaStr);

// Ensure the footer area takes slightly less width padding on mobile
const footerStr1 = `<footer className="absolute bottom-0 left-0 w-full flex flex-col items-center justify-center pb-6 md:pb-8 z-20 shrink-0 gap-4">`;
const newFooterStr1 = `<footer className="absolute bottom-0 left-0 w-full flex flex-col items-center justify-center pb-4 md:pb-8 z-20 shrink-0 gap-3 md:gap-4 pointer-events-none">`;
content = content.replace(footerStr1, newFooterStr1);

// Need to allow pointer-events on footer children
...
const newFooterForm = `<form 
              ...
              >`;
content = content.replace(footerForm, newFooterForm);

const footerBtnWrap = `<div className="flex justify-center">`;
const newFooterBtnWrap = `<div className="flex justify-center pointer-events-auto">`;
content = content.replace(footerBtnWrap, newFooterBtnWrap);

const actionBtnsWrap = `<div className="flex items-center gap-4">`;
const newActionBtnsWrap = `<div className="flex items-center gap-2 md:gap-4 pointer-events-auto">`;
content = content.replace(actionBtnsWrap, newActionBtnsWrap);
...
const newSessionBtn = `<button
            ...
            className={\`
              ...
              \${`;
content = content.replace(sessionBtn, newSessionBtn);
