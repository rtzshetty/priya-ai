...
const originalHeaderStr = `<header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
...
if (content.includes(originalHeaderStr)) {
  content = content.replace(originalHeaderStr, newHeaderStr);
  console.log("Header replaced successfully!");
} else {
  console.error("Header string not found in App.tsx!");
}

// Ensure the chat container is adjusted for mobile header height
...
const newChatAreaStr = `<div className="absolute inset-x-0 top-32 md:top-24 bottom-32 overflow-y-auto px-6 md:px-12 pointer-events-none z-10 custom-scrollbar">`;
